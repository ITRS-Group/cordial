...
type RequestGetListResponse struct {
	...
	Requests []*json.RawMessage `json:"requests"` // each response is just plain JSON, so we can unmarshal it into a config object for easier handling of dynamic fields
}
